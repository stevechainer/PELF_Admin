...
import { updateHistory, deleteHistory } from '../../history/historySlice'
...
function ConfirmationModalBody({ closeModal, extraObject }) {
  ...
  const proceedWithYes = async () => {
    if (type === CONFIRMATION_MODAL_CLOSE_TYPES.GAME_DELETE) {
      ...
      dispatch(updateGameAction(_game))
      dispatch(showNotification({ message: extraObject.toast, status: 1 }))
    } else if (type === CONFIRMATION_MODAL_CLOSE_TYPES.HISTORY_DELETE) {
      console.log('delete history')
      dispatch(deleteHistory(extraObject.history))
    } else if (type === CONFIRMATION_MODAL_CLOSE_TYPES.HISTORY_CLAIM) {
      console.log('claim history')
      dispatch(updateHistory(extraObject.history))
      dispatch(showNotification({ message: extraObject.toast, status: 1 }))
    }
    closeModal()
  }
  ...
  return (
    <>
      ...
      <div className="flex wrapper gap-1 modal-action mt-8">
        ...
        <button className="btn btn-outline w-25" onClick={() => closeModal()}>
          Cancel
        </button>
      </div>
    </>
