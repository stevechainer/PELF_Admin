...
const getGameStatus = (game) => {
  ...
  if (currentTime <= game.startDate)
    return <div className="badge badge-primary">Pending...</div>
  if (currentTime <= game.endDate && currentTime > game.startDate)
    return <div className="badge badge-secondary">Progressing...</div>
  if (currentTime > game.endDate)
...
const getGameCtrlButton = (game) => {
  ...
  if (currentTime <= game.closeTime) return <InfoGameButton game={game} />
  return (
    <>
                ...
                {games.map((game, idx) => {
                  return (
                    <tr key={idx}>
                      ...
                      <td>{game.lockPrice}</td>
                      <td>
                        {convertDateFormat(
                          new Date(game.startDate).toUTCString()
                        )}
                      </td>
                      <td>
                        {convertDateFormat(
                          new Date(game.endDate).toUTCString()
                        )}
                      </td>
                      <td>{game.ownerPercent}%</td>
                      <td>{game.burnPercent}%</td>
                      <td>{getTotalAmount()}</td>
                      <td>{game.maxAmountPerUser}</td>
                      <td>{game.minAmountPerUser}</td>
