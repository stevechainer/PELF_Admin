...
export const getGamesContent = createAsyncThunk('game-data', async () => {
	const response = await axios.post(config.serverUrl + '/api/game-data', {approved: false, conclude:false})
...
export const deleteGameAction = createAsyncThunk('game-delete', async (gameId) => {
	const response = await axios.post(config.serverUrl + '/api/game-delete', {_id : gameId})
	return response.data;
})
